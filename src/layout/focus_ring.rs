...
use jiji_config::{CornerRadius, Gradient, GradientRelativeTo};
...
impl FocusRing {
    pub fn new(config: jiji_config::FocusRing) -> Self {
        Self {
    ...
    pub fn update_config(&mut self, config: jiji_config::FocusRing) {
        self.config = config;
    }
    ...
    pub fn config(&self) -> &jiji_config::FocusRing {
        &self.config
    }
}
