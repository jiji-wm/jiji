...
struct Thumbnail {
    ...
    clock: Clock,
    config: jiji_config::MruPreviews,
...
impl Thumbnail {
    fn from_mapped(mapped: &Mapped, clock: Clock, config: jiji_config::MruPreviews) -> Self {
        let app_id = with_toplevel_role(mapped.toplevel(), |role| role.app_id.clone());

        let background = FocusRing::new(jiji_config::FocusRing {
            off: false,
            width: 0.,
    ...
    fn animate_move_from_with_config(&mut self, from: f64, config: jiji_config::Animation) {
    ...
    fn animate_open_with_config(&mut self, config: jiji_config::Animation) {
    ...
    fn render<R: NiriRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        config: &jiji_config::RecentWindows,
        mapped: &Mapped,
    ...
    pub fn new(niri: &Niri) -> Self {
        ...
        for (mon, ws_idx, ws) in niri.layout.workspaces() {
            ...
            let on_current_workspace = on_current_output
                && niri.layout.active_view(&mon.output_id()).active_position() == ws_idx;
